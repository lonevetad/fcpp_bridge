...
FUN node_nbr_data_t es_4 (ARGS, 
    ...
    { CODE
    auto current_node_nbrhood_data = make_tuple(nbr_amount, node.uid, node.position());
    node_nbr_data_t loneliest = nbr(CALL,
        current_node_nbrhood_data,
        [&](field<node_nbr_data_t> a){
            return min_hood(CALL, a, current_node_nbrhood_data);
        }
    );
     /* TODO:
        - requires a "nbr" passing a tuple of the ID (for debugging), the nbr_amount and the current position
        - then, get the minimum (min_hood with a comparator, somehow?)
        - if the ID != self's ID, then "follow_target" up until ... ool hit = dist <= option::maximum_movement_step;
        - ->
            SEE BELOW
    */
    if(node.uid != get<1>(loneliest)) {
        //real_t dist =
        follow_target(CALL,
            get<2>(loneliest), // most_crowded_device_in_neighborhood
            static_cast<real_t>(option::maximum_movement_step),
            static_cast<real_t>(option::period_between_bounces)
        );
...
FUN_EXPORT es_4_t = export_list<node_nbr_data_t, real_t>;
...
MAIN() {
    ...
    */
    ...
    if  (option::is_bouncing_randomly){ // C++14 and older
#else
    if constexpr (option::is_bouncing_randomly){ // C++17 and earlier
#endif
        ...
        // 4)
        node_nbr_data_t lonelienest_nbr_to_chase_data = es_4(CALL, nbr_amount);

        node.storage(node_nbr_loneliness{})     = static_cast<int>(             get<0>(lonelienest_nbr_to_chase_data));
        node.storage(node_nbr_loneliest_id{})   = static_cast<device_t>(        get<1>(lonelienest_nbr_to_chase_data));
        vec<option::dim> nbr_loneliest_position = static_cast<vec<option::dim>>(get<2>(lonelienest_nbr_to_chase_data));
