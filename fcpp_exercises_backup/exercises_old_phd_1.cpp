...
namespace tags {
    ...
    struct node_max_nbr_count{};
    struct node_global_max_nbr_c{};
}
...
const color C_GREEN = color(GREEN);
const color C_BLUE = color(BLUE);
const color C_BLUE_VIOLET = color(BLUE_VIOLET);
const color C_RED = color(RED);
const real_t MAXIMUM_PROXIMITY = communication_range / 4.0;
...
MAIN() {
    ...
    node.storage(node_color{}) = C_GREEN;
    ...
    int neighboursCount = sum_hood(CALL, 1) - 1;
    node.storage(node_neighbour{}) = neighboursCount;
    node.storage(node_size{}) = max(2, min(20, (neighboursCount<<1)) );
    ...
    /*
    // Version: indipendent 
    node.storage(node_max_nbr_count{}) = old(CALL, 0, [&](int a){
        return max(sum_hood(CALL, 1), a);
    }) - 1;
    */
 
    int maxEverLocalNbrCount = old(CALL, 0, [&](int a){
        return max(neighboursCount, a);
    });
    node.storage(node_max_nbr_count{}) = maxEverLocalNbrCount;
    
    if(maxEverLocalNbrCount == neighboursCount){
        node.storage(node_color{}) = C_BLUE;
    }
    ...
    node.storage(node_global_max_nbr_c{}) = nbr(CALL, 0, [&] (field<int> maxNbrCountFromNears){
          return max( max_hood(CALL,maxNbrCountFromNears), sum_hood(CALL,1)-1);
      }
    );
    */
   
    // Version: recycling results
    int maxGlobalNbrCount = nbr(CALL, 0, [&] (field<int> maxNbrCountFromNears){
        return max(maxEverLocalNbrCount, max_hood(CALL,maxNbrCountFromNears));
    });
    node.storage(node_global_max_nbr_c{}) = maxGlobalNbrCount;

    if(maxEverLocalNbrCount == maxGlobalNbrCount){
        node.storage(node_color{}) = C_BLUE_VIOLET;
    }


    // es 4) and 6)

    auto crowdnessLocalNbrs = nbr( CALL, make_tuple( sum_hood(CALL, 1)-1, node.position()));

    //4
    tuple<int, vec<2>> loneliestLocalNodeData = min_hood(CALL, crowdnessLocalNbrs );
    vec<2> posLLN = get<1>(loneliestLocalNodeData);


    //6
    tuple<int, vec<2>> crowdestLocalNodeData = max_hood(CALL, crowdnessLocalNbrs );
    vec<2> posCLN = get<1>(crowdestLocalNodeData);

    auto currentPosition = node.position();
    const real_t mass = 0.03125;
    
    // 4
    auto directionVector = posLLN - currentPosition;
    real_t distance = norm(directionVector);
    if(distance > MAXIMUM_PROXIMITY){
        auto force_to_min =  directionVector / (distance*distance*distance);
        const real_t elasticity = 16.0;
        node.propulsion() = (elasticity*force_to_min) / mass;
        node.storage(node_shape{}) = shape::sphere;
    }else{
        node.propulsion() *= 0;
        node.velocity() *= 0;
        node.storage(node_shape{}) = distance == 0 ? shape::star : shape::cube;
    }

    directionVector = posCLN - currentPosition;
    distance = norm(directionVector);
    if(distance > MAXIMUM_PROXIMITY){
        auto force_away_to_max = - directionVector / (distance*distance*distance);
        const real_t elasticity = 32.0;
        node.propulsion() += (elasticity*force_away_to_max) / mass;
    }
}
//! @brief Export types used by the main function (update it when expanding the program).
FUN_EXPORT main_t = export_list<int, int, int, int, tuple<int, vec<2>> >;
...
using store_t = tuple_store<
    ...
    node_shape,                 shape,
    node_neighbour,             int,
    node_max_nbr_count,         int,
    node_global_max_nbr_c,      int
>;
